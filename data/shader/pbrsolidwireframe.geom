...
void main()
{
    ...
    if (gs_out.configuration == 7) return;
    ...
    if (gs_out.configuration == 0) {
        ...
        // Now add this perpendicular distance as a per-vertex property in
        // addition to the position and normal calculated in the vertex shader.

        // Vertex 0 (a)
        gs_out.edgeA = vec4(ha, 0.0, 0.0, 0.0);
        gs_out.normal = eyeNormal[0];
        gs_out.position = eyePosition[0];
        gs_out.texCoord = texCoord[0];
        gs_out.color = vcolor[0];
        gs_out.worldPosition = worldPosition[0];
        gs_out.worldNormal = worldNormal[0];
        gl_Position = gl_in[0].gl_Position;
        EmitVertex();

        // Vertex 1 (b)
        gs_out.edgeA = vec4(0.0, hb, 0.0, 0.0);
        gs_out.normal = eyeNormal[1];
        gs_out.position = eyePosition[1];
        gs_out.texCoord = texCoord[1];
        gs_out.color = vcolor[1];
        gs_out.worldPosition = worldPosition[1];
        gs_out.worldNormal = worldNormal[1];
        gl_Position = gl_in[1].gl_Position;
        EmitVertex();

        // Vertex 2 (c)
        gs_out.edgeA = vec4(0.0, 0.0, hc, 0.0);
        gs_out.normal = eyeNormal[2];
        gs_out.position = eyePosition[2];
        gs_out.texCoord = texCoord[2];
        gs_out.color = vcolor[2];
        gs_out.worldPosition = worldPosition[2];
        gs_out.worldNormal = worldNormal[2];
