...
void main()
{
	...
	for (int i = 0; i < MAX_LIGHTS; ++i) {
		...
		lo += (kd * material.albedo / pi + brdf) * radiance * max(dot(vertNormal_w, lightDir_w), 0.0f);
	}

	vec3 ambient = vec3(0.03f) * material.albedo * material.ao; // Add some ambient lighting
